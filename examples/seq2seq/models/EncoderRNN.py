...
class EncoderRNN(BaseRNN):
    ...
    def forward(self, input_var, input_lengths=None,k=None,m=None):
        """
        Applies a multi-layer RNN to an input sequence.
        ...
        if k != None and m !=None:  
            baseline = torch.zeros(self.embedding(input_var).shape)
            self.diff = self.embedding(input_var) - baseline
            self.embedded = baseline + (k/m) * self.diff
        else:
            self.embedded = self.embedding(input_var) 
        embedded = self.input_dropout(self.embedded)
